  ...
  def translate(labelColumn: String, featuresColumn: String, parallelism: String, log: Logger,
    ...
    try {
      ...
      try {
        ...
        val tempM = lightgbmlib.LGBM_BoosterSaveModelToStringSWIG(boosterPtr.get, -1, bufferLengthPtrInt64,
          bufferOutLengthPtr)
        ...
        val model =
          if (bufferOutLength > bufferLength) {
            lightgbmlib.LGBM_BoosterSaveModelToStringSWIG(boosterPtr.get, -1, bufferOutLengthPtr, bufferOutLengthPtr)
          } else tempM
        ...
        if (boosterPtr.isDefined) {
          LightGBMUtils.validate(lightgbmlib.LGBM_BoosterFree(boosterPtr.get), "Finalize Booster")
        }
      }
    } finally {
      // Free dataset
      if (datasetPtr.isDefined) {
        LightGBMUtils.validate(lightgbmlib.LGBM_DatasetFree(datasetPtr.get), "Finalize Dataset")
      }
    }
  }
