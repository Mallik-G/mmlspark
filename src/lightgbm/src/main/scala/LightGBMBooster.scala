...
class LightGBMBooster(val model: String) extends Serializable {
  ...
  protected def predictForCSR(sparseVector: SparseVector): Double = {
    ...
    try {
      ...
      if (scoredDataOutPtr.isDefined) {
        lightgbmlib.delete_doubleArray(scoredDataOutPtr.get)
      }
    }
  }
  ...
  protected def predictForMat(row: Array[Double]): Double = {
    ...
    try {
      ...
      LightGBMUtils.validate(lightgbmlib.LGBM_BoosterPredictForMat(boosterPtr, data.get._1, data64bitType,
        numRows_int32_tPtr, numCols_int32_tPtr, isRowMajor, lightgbmlibConstants.C_API_PREDICT_RAW_SCORE,
        -1, datasetParams, scoredDataLength_int64_tPtr, scoredDataOutPtr.get), "Booster Predict")
      lightgbmlib.doubleArray_getitem(scoredDataOutPtr.get, 0)
    } finally {
      // Delete the input row
      if (data.isDefined) {
